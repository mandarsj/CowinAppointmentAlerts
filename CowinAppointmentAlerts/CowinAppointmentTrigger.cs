...
namespace CowinAppointmentAlerts
{
    public class CowinAppointmentTrigger
    {
        ...
        private static async Task Run(ILogger logger)
        {
            ...
            // var settings = alertDetails.alertSpecs;
            ...
            foreach (var user in users)
            {
                ...
                client.DefaultRequestHeaders.UserAgent.TryParseAdd("Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_4) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4103.61 Safari/537.36");
                ...
                foreach (var pin in pincodes)
                {
                    ...
                    {
                        var appointmentTomorrow = await client.GetStringAsync($"https://cdn-api.co-vin.in/api/v2/appointment/sessions/public/findByPin?pincode={pintosend.ToString()}&date={DateTime.Now.AddDays(1).ToString("dd-MM-yyyy")}");
                        var sessions = JsonSerializer.Deserialize<allsessions>(appointmentTomorrow);

                        //var appointmentToday = await client.GetStringAsync($"https://cdn-api.co-vin.in/api/v2/appointment/sessions/public/findByPin?pincode={key}&date={DateTime.Now.ToString("dd-MM-yyyy")}");
                        //var sessionsToday = JsonSerializer.Deserialize<allsessions>(appointmentToday);
                        ...
                        if (sessions.sessions.Count > 0)
                        {
                            ...
                            var validsessions = sessions.sessions.Where(p => p.available_capacity_dose1 > 0 || p.available_capacity_dose2>0);
            ...
            {
                 //"Data Source=LIN22004804\\SQLEXPRESS;Persist Security Info=False;Integrated Security=true;Initial Catalog=CowinNotifDb";
